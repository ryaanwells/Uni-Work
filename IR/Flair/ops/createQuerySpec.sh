...
echo $JAVA_HOME/bin/java -Xmx512M $JAVA_OPTIONS $FLAIR_OPTIONS -Dflair.etc=$FLAIR_ETC -Dflair.home=$FLAIR_HOME -cp $FLAIR_CLASSPATH uk.ac.gla.mir.flair.application.CreateQuerySpec $@

$JAVA_HOME/bin/java -Xmx512M $JAVA_OPTIONS $FLAIR_OPTIONS \
	 -Dflair.etc=$FLAIR_ETC \
       -Dflair.home=$FLAIR_HOME \
	 -cp $FLAIR_CLASSPATH uk.ac.gla.mir.flair.application.CreateQuerySpec $@